...
class ServiceImpl {
    ...
    private var realId = 1
    private val restTemplate = RestTemplate()

    fun getByName(name: String) = Pokemon.getByName(name)

    fun getPaginated(pageNo: Int, pageSize: Int): PagingResponse<PokeLocal> {
    ...
    fun updateNickNamePokemon(pokemonRequest: PokemonRequest): ResponseEntity<Any>{
        ...
        return try {
            ...
            ResponseEntity(Pokemon.getByName(pokemonRequest.name), HttpStatus.OK)
        } catch (ex: SQLException) {
            ResponseEntity(ex.message, HttpStatus.NOT_MODIFIED)
        }
    }

    fun updateFavoritePokemon(name: String): ResponseEntity<Any?> {
        ...
        return try {
            ...
            ResponseEntity(Pokemon.getByName(name), HttpStatus.OK)
        } catch (ex: SQLException) {
            ResponseEntity(ex.message, HttpStatus.NOT_MODIFIED)
        }
    }
